...
public class SerializeFactoryTest
{
    ...
    public void DefaultSerializerFactory_CreateForType_ReturnsCorrectSerializer(Type serializeable, Type serializerType)
    {
        ...
        Microsoft.VisualStudio.TestTools.UnitTesting.Assert.IsInstanceOfType(serializer, serializerType);
    }
    ...
    public void DefaultDeserializerFactory_CreateForType_ReturnsCorrectDeserializer(Type deserializeable, Type deserializerType)
    {
        ...
        Microsoft.VisualStudio.TestTools.UnitTesting.Assert.IsInstanceOfType(deserializer, deserializerType);
    }
    [TestCase(42)]
    [TestCase(42.0)]
    [TestCase(42.0f)]
    ...
    public void SerializeAndDeserializeBack_OriginAndDeserializedAreEqual(object val)
    {
        ...
        Assert.That(val, Is.EqualTo(deserialized));
    }
    ...
    public void ProtoContract_SerializeAndDeserializeBack_OriginAndDeserializedAreEqual()
    {
        ...
        Assert.That(origin.IsSameTo((Team)deserialized));
    }
    ...
    public void NullableValuableInt_OriginAndDeserializedAreEqual()
    {
        int? origin = 42;
        var deserialized = SerializeAndDeserializeBack(origin);
        Assert.That(deserialized, Is.EqualTo(origin));
    }
    [TestCase(42)]
    [TestCase(0)]
    ...
    public void ArrayOfStrings_SerializeAndDeserializeBack_OriginAndDeserializedAreEqual()
    {
        ...
        Assert.That(origin.Length, Is.EqualTo(deserialized.Length));

        for (int i = 0; i < origin.Length; i++)
        {
            Assert.That(origin[i], Is.EqualTo(deserialized[i]));
        }
    }
    [Test]
    ...
    public void ComplexTypesSequence_SerializeAndDeserializeBack_OriginAndDeserializedAreEqual()
    {
        ...
        Assert.Multiple(() =>
        {
            Assert.That(origin.Length == deserialized.Length);
        ...
        var coll2 = ((IEnumerable)deserialized[2]).Cast<int>().ToArray();

        Assert.That(coll1.Length, Is.EqualTo(coll2.Length));

        for (int i = 0; i < coll1.Length; i++)
        {
            Assert.That(coll1[i], Is.EqualTo(coll2[i]));
        }
    }
