...
namespace TNT.Core.Transport
{
    public class Transporter
    {
        ...
        public Task WriteAsync(MemoryStream message)
        {
            _sendStreamManager.PrepareForSending(message);
            if (!message.TryGetBuffer(out var buffer))
                throw new InvalidOperationException();
            return Channel.WriteAsync(buffer.Array, (int)message.Position, (int)(message.Length - message.Position));
        }
